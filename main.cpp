...
int main(int argc, char* argv[]) {
    ...
    while (true) {
        ...
        const u_char *payload = packet + sizeof(struct libnet_ethernet_hdr) + ip_hdr_len + tcp_hdr_len;
        int payload_len = header->caplen - (sizeof(struct libnet_ethernet_hdr) + ip_hdr_len + tcp_hdr_len);

        if(ip_hdr->ip_p != IPPROTO_TCP) continue;

        printf("%s",ether_ntoa((const struct ether_addr *)eth_hdr->ether_shost));
        printf("-> %s \n",ether_ntoa((const struct ether_addr *)eth_hdr->ether_dhost));
        printf("%s : %u ",inet_ntoa(ip_hdr->ip_src), ntohs(tcp_hdr->th_sport));
